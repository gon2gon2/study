...
class Visualizer:
    def __init__(self, show_step=10,imgae_size=30):
    ...
    def show(self, inputsG, inputsD_real, inputsD_fake):
        ...
        if self.step == self.show_step:
            ...
            hr_image = self.transform(inputsD_read[i])
            fake_hr_image = self.transform(inputsD_fake[i])
            
            if self.lr_image_ph in None:
